...
def generate():
    ...
    # Compute all stats
    stats = {"total_practices": 0, "automated": 0, "manual": 0,
    ...
    for fam_id, family in sorted(pd["families"].items(), key=_numeric_key):
        ...
        name = family["name"]
        ds = {"name": name, "practices": 0, "automated": 0, "manual": 0,
    ...
    a("CMMC Cloud Compliance Scanner  - Certified CMMC Assessor (CCA) Reference")
    a('</p>')
    a('</div></div>')
    ...
    a("The CMMC Cloud Compliance Scanner evaluates DIB contractor cloud environments against ")
    a("all 110 NIST SP 800-171 Rev 2 practices. For each practice, the scanner maps its NIST SP 800-171A ")
    a("assessment objectives to cloud-specific API checks across AWS, Azure, and GCP.")
    a('</p>')
    a('<p class="help-section-desc">')
    a("This document explains <strong>how</strong> each practice is evaluated, <strong>which</strong> cloud APIs are queried, ")
    a("<strong>why</strong> each check maps to specific assessment objectives, and <strong>what</strong> CCAs must do ")
    a("for the 39 practices that require manual review.")
    a('</p>')

    # Key stats grid
    a('<div class="help-stats-grid">')
    stat_items = [
        ("Practices", str(stats["total_practices"]), "NIST 800-171", "practices"),
        ("Objectives", str(stats["total_objectives"]), "NIST 800-171A", "objectives"),
        ("Automated", str(stats["automated"]), "practices", "automated"),
        ("Manual", str(stats["manual"]), "practices", "manual"),
    ...
    chain_steps = [
        ("shield-alt", "CMMC 2.0 Level", "L1 / L2 / L3 certification tier"),
        ("book", "NIST SP 800-171 Practice", "One of 110 security requirements"),
        ("clipboard-check", "800-171A Assessment Objective", "Specific &ldquo;determine if&rdquo; statement"),
    ...
    for src, ver, purpose, url, url_label in [
        ("NIST SP 800-171 Rev 2", "Feb 2020", "110 security practices across 14 families",
         "https://csrc.nist.gov/publications/detail/sp/800-171/rev-2/final", "csrc.nist.gov"),
        ("NIST SP 800-171A", "Jun 2018", "319 assessment objectives (&quot;determine if&quot; statements)",
         "https://csrc.nist.gov/publications/detail/sp/800-171a/final", "csrc.nist.gov"),
        ("NIST SP 800-172", "Feb 2021", "Enhanced security practices for Level 3",
         "https://csrc.nist.gov/publications/detail/sp/800-172/final", "csrc.nist.gov"),
        ("FAR 52.204-21", "2016", "17 basic safeguarding practices for Level 1",
         "https://www.acquisition.gov/far/52.204-21", "acquisition.gov"),
        ("CMMC 2.0 Model", "Dec 2021", "Three-level certification model",
         "https://dodcio.defense.gov/CMMC/", "dodcio.defense.gov"),
        ("AWS Config Rules", "Current", "~200 rules mapped to NIST 800-171",
         "https://docs.aws.amazon.com/config/latest/developerguide/operational-best-practices-for-nist_800-171.html", "docs.aws.amazon.com"),
        ("Azure Policy", "Current", "~200 policy definitions for NIST 800-171 R2",
         "https://learn.microsoft.com/en-us/azure/governance/policy/samples/nist-sp-800-171-r2", "learn.microsoft.com"),
        ("GCP CIS Benchmark", "Current", "GCP security controls aligned to NIST practices",
    ...
    a('<p class="help-section-desc">Each of the 319 NIST SP 800-171A assessment objectives is classified into one of three tiers:</p>')
    ...
    a(f'<tr><td><span class="tag tag-manual">Tier 2</span></td><td>Partially Automatable</td><td>{stats["obj_partial"]}</td><td>Check provides evidence; CCA verifies organizational context</td></tr>')
    a(f'<tr><td><span class="tag tag-not-met">Tier 3</span></td><td>Not Automatable</td><td>{stats["obj_false"]}</td><td>Flagged as Documentation Required; CCA assesses manually</td></tr>')
    a('</tbody></table>')
    a('</div></div>')
    ...
    a('<p class="help-section-desc">')
    a("The table below shows the scanner's coverage across all 14 CMMC domains. Each domain is broken down by the number of ")
    a("NIST 800-171 practices, how many are automated vs. manual, the total 800-171A assessment objectives, and the cloud-specific ")
    a("checks implemented for each provider. The <strong>Automation Rate</strong> bar shows the percentage of practices in each domain ")
    a("that are fully automated by the scanner.")
    a('</p>')
    a('<div class="help-matrix-legend">')
    a('<span class="help-legend-item"><span class="help-legend-dot" style="background:#5B63D3"></span> Practices &amp; Objectives</span>')
    ...
    a('<thead><tr><th>Domain</th><th>Name</th><th>Practices</th><th>Auto</th><th>Manual</th><th>Objectives</th><th>AWS</th><th>Azure</th><th>GCP</th><th>Automation Rate</th></tr></thead>')
    a('<tbody>')
    ...
    for dc in domain_order:
        if dc in domain_stats:
            ...
            a(f'<tr>'
              ...
              f'<td class="cm-practices">{d["practices"]}</td>'
    ...
    a(f'<tr class="table-total">'
      ...
      f'<td class="cm-practices"><strong>{stats["total_practices"]}</strong></td>'
    ...
    # --- Domain-by-Domain Reference (all 110 practices) ---
    a('<div class="card mb-lg">')
    a('<div class="card-body">')
    a('<h3 class="help-section-title">Complete Practice Reference (All 110 Practices)</h3>')
    a('<p class="help-section-desc">')
    a('Expand each domain below to see all practices with their assessment objectives, automated checks, ')
    ...
    for fam_id, family in sorted(pd["families"].items(), key=_numeric_key):
        ...
        practices = sorted(family["practices"].items(), key=_numeric_key)
        ...
        a(f'<span class="help-accordion-title">{esc_html(name)}  - {ds["practices"]} practices ({ds["automated"]} auto, {ds["manual"]} manual)</span>')
        a('</div>')
        a('<div class="help-accordion-body"><div class="help-accordion-body-inner">')

        for pid, p in practices:
            ...
            a(f'<div class="help-practice-card">')
            a(f'<h5 class="help-practice-id">{pid} '
              f'<span class="tag tag-{level.lower()}">{level}</span> '
              f'<span class="tag tag-{auto_tag}">{auto_label}</span></h5>')
            a(f'<p class="help-practice-req">{esc_html(p["requirement"])}</p>')
            ...
            if not is_auto and p.get("manual_guidance"):
                a(f'<div class="help-info-box"><strong>CCA Guidance:</strong> {esc_html(p["manual_guidance"])}</div>')
    ...
    # --- CCA Manual Assessment Guide ---
    a('<div class="card mb-lg">')
    a('<div class="card-body">')
    a('<h3 class="help-section-title">CCA Manual Assessment Guide</h3>')
    a('<p class="help-section-desc">')
    a(f'For the {stats["manual"]} practices requiring manual assessment, the scanner flags them as ')
    a('"Manual Review Required." The CCA must independently evaluate these using the guidance below. ')
    a('For each practice, this guide provides the 800-171A objectives, assessment steps, and evidence artifacts to request.')
    a('</p>')
    a('<div class="help-accordion">')
    ...
    for fam_id, family in sorted(pd["families"].items(), key=_numeric_key):
        ...
        name = family["name"]
        manual_practices = {
            pid: p for pid, p in sorted(family["practices"].items(), key=_numeric_key)
        ...
        a('<svg class="help-accordion-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>')
        a(f'<span class="tag tag-domain">{domain}</span>')
        a(f'<span class="help-accordion-title">{esc_html(name)} ({len(manual_practices)} manual practices)</span>')
        a('</div>')
        a('<div class="help-accordion-body"><div class="help-accordion-body-inner">')

        for pid, p in sorted(manual_practices.items(), key=_numeric_key):
            a(f'<div class="help-practice-card">')
            a(f'<h5 class="help-practice-id">{pid} <span class="tag tag-{p.get("level", "L2").lower()}">{p.get("level", "L2")}</span></h5>')
            a(f'<p class="help-practice-req">{esc_html(p["requirement"])}</p>')

            # Assessment objectives
            objs = p.get("objectives", {})
            if objs:
                a('<div class="help-practice-objectives">')
            ...
            if mg:
                a(f'<div class="help-info-box"><strong>CCA Guidance:</strong> {esc_html(mg)}</div>')
    ...
    a("(<code>config/nist_practices.json</code> and <code>config/checks/*.json</code>). ")
